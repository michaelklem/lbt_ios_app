...
@implementation UserInputTextView
@synthesize limited;
...
+(UserInputTextView*)viewFromNib:(id)owner {
    ...
    if (IsIdiomPad) {
        nibName = @"UserInputTextView-iPad";
    } else {
        nibName = @"UserInputTextView-iPhone";
    }
    NSArray *bundle = [[NSBundle mainBundle] loadNibNamed:nibName
                                                    owner:owner options:nil];
    for (id object in bundle) {
        if ([object isKindOfClass:[UserInputTextView class]]) {
            [(UserInputTextView*)object setLimited:YES];
            return object;
        }
    }   
    return nil;
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        // Initialization code
        textLeftLabel.text = [NSString stringWithFormat:@"%i characters left", 400-textView.text.length];
    }
    return self;
}
